...
hls = 0 # hls formated image
src = 0 # orginal image copy
...
def clr_segment(hls,lower_range,upper_range):
    mask_in_range = cv2.inRange(hls,lower_range,upper_range) ## segmenting the image
    
    mask_dilated = cv2.morphologyEx( # image dilating width increase of edges
        mask_in_range,
        op=cv2.MORPH_DILATE,
